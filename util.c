...
Result util_import_seed(u32 *responseCode, u64 titleId)
{
   ...
   if (fsPath != NULL)
   {
      ...
      if (R_SUCCEEDED(res = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, fsMakePath(PATH_EMPTY, ""), *fsPath,
                            FS_OPEN_READ, 0)))
      {
         u32 bytesRead = 0;
      ...
      if (R_FAILED(res))
      {
         ...
         if (region <= CFG_REGION_TWN)
         {
            ...
            char url[128];
            snprintf(url, 128, "https://kagiya-ctr.cdn.nintendo.net/title/0x%016llX/ext_key?country=%s", titleId,
...
FS_MediaType util_get_title_destination(u64 titleId)
{
   ...
   return platform == 0x0003 || (platform == 0x0004 && ((category & 0x8011) != 0 || (category == 0x0000
...
Result util_get_cia_file_smdh(SMDH *smdh, Handle handle)
{
   ...
   if (smdh != NULL)
   {
      ...
      if (R_SUCCEEDED(res = FSFILE_Read(handle, &bytesRead, 0, header, sizeof(header))) && bytesRead == sizeof(header))
      {
         ...
         if (metaSize >= 0x3AC0)
            res = FSFILE_Read(handle, &bytesRead, headerSize + certSize + ticketSize + tmdSize + contentSize + 0x400, smdh,
                              sizeof(SMDH));
...
bool util_filter_tickets(void *data, const char *name, u32 attributes)
{
   ...
   size_t len = strlen(name);
   return (len >= 4 && strncasecmp(name + len - 4, ".tik", 4) == 0) || (len >= 5
          && strncasecmp(name + len - 5, ".cetk", 5) == 0);
}
...
SMDH_title *util_select_smdh_title(SMDH *smdh)
{
   ...
   if (R_SUCCEEDED(CFGU_GetSystemLanguage((u8 *) &systemLanguage)))
      utf16_to_utf8((uint8_t *) shortDescription, smdh->titles[systemLanguage].shortDescription,
                    sizeof(shortDescription) - 1);
